...
*/
MiniSumo::MiniSumo(char name[]) : Robot(name) {
  // MotorGroup class drives the motors
  motorGroup = new MotorGroupDifferential(this, mrm_mot4x3_6can, 0, mrm_mot4x3_6can, 1, mrm_mot4x3_6can, 2, mrm_mot4x3_6can, 3); // Motor driver je za 4 motora, iako robot koristi samo 2
  ...
  // mrm_8x8a->actionSet(actionPause, 0); // Button 1 starts RCJ Line.
  ...
  //mrm_mot4x3_6can->directionChange(0); // Uncomment to change MOTOR 1 rotation direction - DESNI KOTAČ
  //mrm_mot4x3_6can->directionChange(1); // Uncomment to change MOTOR 2 rotation direction - LIJEVI KOTAČ

  // DIGITAL or ANALOG I/O FOR ADDITIONAL DEVICES
  pinMode(16, INPUT); // IR SENZOR (16)
...
*/
bool MiniSumo::button(uint8_t number) {
...
*/
void MiniSumo::display(uint8_t image) {
...
/** Display 8x8 text
  @image - image's number
*/
void MiniSumo::display(char* text) {
  mrm_8x8a->text(text);
}

uint16_t MiniSumo::left(uint8_t sampleCount, uint8_t sigmaCount) {
...
uint16_t MiniSumo::frontLeft(uint8_t sampleCount, uint8_t sigmaCount) {
...
uint16_t MiniSumo::frontRight(uint8_t sampleCount, uint8_t sigmaCount) {
...
uint16_t MiniSumo::right(uint8_t sampleCount, uint8_t sigmaCount) {
...
/** Store 8x8 image to 8x8 LED's internal memory
  ...
  @image - image's number
*/
void MiniSumo::store(uint8_t red[], uint8_t green[], uint8_t image) {
