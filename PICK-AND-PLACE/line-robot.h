...
/* All the Action-classes have to be forward declared here (before LineRobot) as LineRobot declaration uses them. The other option would be
  not to declare them here, but in that case Action-objects in LineRobot will have to be declared as ActionBase class, forcing downcast later in code, if
  derived functions are used.*/
class ActionPause;
class ActionMotorShortTest;

/** Robot for LineRobot, a class derived from the base Robot class.
*/
class LineRobot : public Robot {
  ...
  public:
    ...
    */
    LineRobot(char name[] = (char*)"SUMO"); // Maximum 15 characters
...
/** Actions specific for a LineRobot robot will be defined here. They are all derived from ActionBase class (inheriting its methods and properties).
  ...
  The second one is a 3-letter shortcut that is displayed in command menu. For example "lin" will be displayed for starting the Rescue LineRobot run. When action is not supposed to be started from menu,
  it can be an empty string.
  The third parameter is a name of the action, again displayed in menu. For "lin", the name is "RCJ LineRobot", causing menu entry "LineRobot - RCJ LineRobot" to be displayed. Again, use empty string
  for no-menu actions.
...
class ActionMotorShortTest : public ActionBase {
    void perform() {
      ((LineRobot*)_robot)->motorShortTest();
    }
  public:
